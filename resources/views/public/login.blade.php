...
<div class="max-w-sm mx-auto px-4 py-14">
    <div class="text-center mb-6">
        ...
        <p class="mt-1 text-sm text-muted">Masuk untuk mengakses produk & kelas Anda.</p>
    </div>
    <div class="jm-card p-6">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-3.5">@csrf
            <div>
                <label class="block text-sm font-semibold mb-1.5">Email</label>
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="nama@email.com" class="jm-input">
            </div>
            <div>
                <label class="block text-sm font-semibold mb-1.5">Password</label>
                <input type="password" name="password" required placeholder="••••••••" class="jm-input">
            </div>
            <div class="text-right -mt-1">
                <a href="{{ route('password.request') }}" class="text-xs font-semibold text-muted hover:text-accent-c transition">Lupa password?</a>
            </div>
            <button class="w-full btn-accent text-white font-bold rounded-xl py-3.5 min-h-[48px] shadow-cta hover:opacity-90 transition">Masuk</button>
        </form>

        @if ($waReady || $googleReady)
            ...
            <div class="grid gap-2.5">
                @if ($waReady)
                    <a href="{{ route('login.otp') }}" class="rounded-xl border border-ink/15 py-3 text-center font-semibold min-h-[48px] flex items-center justify-center gap-2 hover:border-ink/30 hover:bg-stone-50 transition">💬 Masuk dengan WhatsApp</a>
                @endif
                @if ($googleReady)
    ...
    <p class="mt-5 text-center text-sm text-muted">Belum punya akun? <a href="{{ route('register') }}" class="text-accent-c font-bold hover:underline">Daftar gratis</a></p>
</div>
@endsection
